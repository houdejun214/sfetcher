...
object ShangbiaoCrawler extends AbstractApp {
  ...
  val layer0 =
    Pattern("http://sbcx.saic.gov.cn:9080/tmois/wsggcx_getGgaoMainlate.xhtml")
    .links(Seq(
  ...
  val layer1 =
    Pattern("http://sbcx.saic.gov.cn:9080/tmois/wsggcx_getImageInputSterem.xhtml*")
    .datum("shangbiao", Seq(
      "img".on("{$url}")
    )).entry()

  var entry =
    Entry("POST http://sbcx.saic.gov.cn:9080/tmois/wsggcx_getGgaoMainlate.xhtml")
