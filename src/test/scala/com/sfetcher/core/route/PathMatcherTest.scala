...
class PathMatcherTest extends Specification {
  ...
  "PathTest" should {
    "match" in {
      ...
      val path = PathMatcher("http://www.amazon.com/s/*")
    ...
    "match2" in {
      ...
      val path = PathMatcher("https://hanxierka.tmall.com/category-{id}.htm?*")
      path.`match`(url) must beTrue
    ...
    "path" in {
      val url: String = "https://hanxierka.tmall.com/category-1046631575.htm?search=y&catName=%B4%BA%CF%C4%D7%B0%D0%C2%C6%B7%C7%F8"
      val path = PathMatcher("https://hanxierka.tmall.com/category-{name}-{id}.htm?*")
