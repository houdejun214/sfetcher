...
public class FashionBlogResultChecker{
	...
	public FashionBlogResultChecker(Configuration conf){
		this.conf = conf;
	...
	public Boolean saveResult(final String key,final String val){
		String sql="update "+RunStateTableName+" set value=:value where siteName=:source and `key`=:key";
		Map<String,Object> parameters = new HashMap<String,Object>();
		...
		if(result<=0){
			try {
				sql="insert into "+RunStateTableName+" (`key`,siteName,value) values(:key,:source,:value)";
	...
	public Boolean checkKeyHaveSuccess(final String key,final String val){
		String sql="select count(*) from "+RunStateTableName+" where siteName=:source and `key`=:key and value>=:value";
		Map<String,Object> parameters = new HashMap<String,Object>();
		parameters.put("key", key);
		parameters.put(Constants.SOURCE, "fashionBlogs");
		parameters.put("value", val);
