			...
			try {
				...
				if (this.iRecursionCount==0 && sline.matches("(.*)\"url_encoded_fmt_stream_map\":(.*)")) {
					...
					for (String urlString : urlStrings) {
						...
						try {
							...
							sResolutions = sResolutions.concat(
									fmtUrlPair[1].equals("37")?"1080p mpg, ":		// HD
									fmtUrlPair[1].equals("22")?"720p mpg, ":		// HD
									fmtUrlPair[1].equals("35")?"480p mpg, ":		// SD
									fmtUrlPair[1].equals("18")?"360p mpg, ":		// SD
									fmtUrlPair[1].equals("34")?"360p mpg, ":		// SD
									fmtUrlPair[1].equals("36")?"240p mpg, ":		// LD
									fmtUrlPair[1].equals("17")?"114p mpg, ":		// LD
										
									fmtUrlPair[1].equals("46")?"1080p flv/webm, ":	// HD 
									fmtUrlPair[1].equals("45")?"720p flv/webm, ":	// HD
									fmtUrlPair[1].equals("44")?"480p flv/webm, ":	// SD
									fmtUrlPair[1].equals("43")?"360p flv/webm, ":	// SD
									fmtUrlPair[1].equals( "5")?"240p flv/webm, ":	// LD
									"unknown resolution! (".concat(fmtUrlPair[1]).concat(")"));
					...
					case 1:	// LD
						...
						this.sFilenameResPart = "(LD)"; // adding LD to filename because HD-Videos are almost already named HD (?)
						if (JFCMainClient.getBmpgbuttonstate()) {
