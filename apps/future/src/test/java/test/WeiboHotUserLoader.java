...
public class WeiboHotUserLoader {
	...
	private static List Fashion = Arrays.asList("娱乐", "时尚", "艺术", "动漫", "育儿",
			"文学", "健康");
	private static List Times = Arrays.asList("体育", "财经", "传媒", "房产", "商业",
			"军事", "公益");
	private static List Tech = Arrays.asList("汽车", "科技", "教育", "科普");

	private static HBaseClient client = HBaseClientFactory
			.getClientWithCustomSeri("next-2", "thot_type");
	...
	public static void main(String[] args) {
		...
		// List<Element> elements = JsoupUtils.getList(doc,
		// ".hottabs_list a:contains(媒体)");
		List<Element> elements = JsoupUtils.getList(doc,
				"div.hottabs_list_second[node-type] a:not(.current)");
		for (Element e : elements) {
			String url = JsoupUtils.getLink(e, "a");
			Document tdoc = fetchPage(url);
			// String cur = JsoupUtils.getText(tdoc,
			// ".hottabs_list_second a.current");
	...
	private static List<Map<String, Object>> getUsers(Document tdoc) {
		...
		String cur2 = JsoupUtils
				.getText(tdoc, ".hottabs_list_second a.current");
	...
	private static String getNextPage(Document tdoc, int page) {
		...
		return PatternUtils.replaceMatchGroup("page=(\\d)&", absUrl, 1,
				String.valueOf(page));
	}
