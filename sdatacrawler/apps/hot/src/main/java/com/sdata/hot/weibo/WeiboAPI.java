...
public class WeiboAPI {
	...
	public JSONObject fetchOneTweet(String id) {
		...
		try {
			...
			e.printStackTrace();
			log.error("fetchOneTweet error:" + e.getRequest());
		}
		return null;
	}
	 ...
	 * @return
	 */
	public JSONArray fetchBatchTweets(String ids) {
		List<Object> tweets = new ArrayList<Object>();
		try {
			weibo4j.org.json.JSONObject jsonStatus = Weibo.client.get(WeiboConfig.getValue("baseURL")+ "statuses/show_batch.json",
							new PostParameter[] {
							new PostParameter("ids", ids)}).asJSONObject();
